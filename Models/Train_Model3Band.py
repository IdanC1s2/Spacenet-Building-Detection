...
from Set_Iterators import Data_Iterator_3Band
...
# Set Paths:
train_image_dir = 'E:/Spacenet Database/Project Database/Training Data/image_3B/img'
train_mask_dir = 'E:/Spacenet Database/Project Database/Training Data/mask/img'
val_image_dir = 'E:/Spacenet Database/Project Database/Validation Data/image_3B/img'
val_mask_dir = 'E:/Spacenet Database/Project Database/Validation Data/mask/img'
test_image_dir = 'E:/Spacenet Database/Project Database/Test Data/image_3B/img'
test_mask_dir = 'E:/Spacenet Database/Project Database/Test Data/mask/img'

# Set Data Iterators:
train_generator = Data_Iterator_3Band(batch_size=16, img_size=(128, 128),
                                      input_img_dir=train_image_dir, mask_img_dir=train_mask_dir,
                                      augment=True)

val_generator = Data_Iterator_3Band(batch_size=16, img_size=(128, 128),
                                    input_img_dir=val_image_dir, mask_img_dir=val_mask_dir)

test_generator = Data_Iterator_3Band(batch_size=16, img_size=(128, 128),
...
model_first_5Epochs = Build_UNET_Model(128, 128, 3, Adam_lr=0.001, loss='BinaryCrossEntropy')
history1 = model_first_5Epochs.fit(x=train_generator, epochs=5, validation_data=val_generator, verbose=1)
save_model_history(history1, 'history1', dir='Models/Architecture3')
model_first_5Epochs.save_weights('Models/Architecture3/Model5Epochs_weights.h5', save_format='h5')
show_masks(model_first_5Epochs, save_path='Models/Architecture3/Masks_5_Epochs', bands=3)
##########################################

# Next - we load the saved model - and start using our custom mean_iou loss:
model = Build_UNET_Model(128, 128, 3, Adam_lr=0.001)
model.load_weights('Models/Architecture3/Model5Epochs_weights.h5')


# Train Model and Save:
cb = EarlyStopping(monitor='my_mean_iou', mode='max', patience=10, restore_best_weights=True)
history2 = model.fit(x=train_generator, epochs=15, validation_data=val_generator, verbose=1)
save_model_history(history2, 'history2', dir='Models/Architecture3')
model.save_weights('Models/Architecture3/Model20Epochs_weights.h5', save_format='h5')
show_masks(model, save_path='Models/Architecture3/Masks_80_Epochs')

# Visualize History:
history = merge_histories_in_directory('Models/Architecture3')
